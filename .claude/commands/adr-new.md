...
3. **The three fields that carry the format.**
   - _Fatal hypothesis_ — the single falsifiable belief whose falsity kills the
     decision. One only; about the real world, not an internal intention.
   - _Kill-test_ — the observable, dated signal proving it wrong: what to measure,
     which threshold, when checked, action on breach. Mechanise it as a test, or
     state why it cannot be.
   - _Validation gate_ — the pre-agreed condition that unlocks the next step,
